...
The paper \textit{An Analytically Solvable Principal–Agent Model} by Lars-Göran Mattsson and Jörgen W. Weibull (GEB, 2023) studies a single-period,
...
From this point onward, the paper specializes to particular functional forms for the subutility function \(u\).
...
See Appendix.
\end{proof}
Thus, in period~1 the principal behaves as if the relevant ``effective output'' were \(\pi_i+C_i\), i.e.\ current output plus discounted expected continuation profit.
...
In that case, a joint two-period contract can increase both expected output and expected wage payments, and the agent may end up strictly better off than under repeated one-period contracting.

I begin with the numerical example in the appendix of Mattsson and Weibull. There are three outcomes \(K=\{1,2,3\}\) with gross profits \(\pi\in\{-1,0,1\}\) 
...
for the principal while giving the agent expected utility \(\Ubaselineoneperiod\), i.e.\ the participation constraint almost binds.
...
the principal’s expected total profit is approximately \(\fpeval{round(2*\Pibaselineoneperiod, 3)}\), while the agent’s expected 
total utility equals \(\fpeval{round(2*\Ubaselineoneperiod, 3)}\) (i.e.\ her outside option). If instead the parties can sign a 
...
in the one-period benchmark). If, on the other hand, the parties can sign a two-period contract, the principal again chooses a contract of the same form as before. The optimal parameters are approximately
\[
...
Analytically, the optimal wage schedules retain a simple canonical form. Under risk neutrality, optimal contracts are debt-like (threshold) 
in both periods, with a history-dependent face value in period~2 and a period~1 threshold written on a continuation-adjusted performance measure. 
Under CRRA preferences, the same threshold structure remains, but surplus above the threshold is shared according to the paper’s canonical sharing rule.
